...
if (isset($_POST["kaydet"])) {
    ...
    // Kullanıcı adı ve şifre için minimum uzunluk kontrolü
    if (empty($name) || strlen($name) < 5) {
        $kullaniciadiHata = "Kullanıcı adı en az 5 karakter olmalıdır";
    }
    if (empty($parola) || strlen($parola) < 6) {
        $parolaHata = "Şifre en az 6 karakter olmalıdır";
    }

    // Şifre tekrarını kontrol et
    if ($parola !== $parola_tekrar) {
        $parolaHata = "Şifreler uyuşmuyor";
    }

    if (empty($kullaniciadiHata) && empty($parolaHata)) {
        // SQL enjeksiyonlarına karşı koruma
        $kontrolSorgusu = "SELECT kullanici_adi FROM kullanicilar WHERE kullanici_adi = ?";
        $stmt = mysqli_prepare($baglanti, $kontrolSorgusu);
        ...
        if (mysqli_num_rows($sonuc) > 0) {
            ...
            // Veritabanına ekle
            $ekle = "INSERT INTO kullanicilar (kullanici_adi, parola) VALUES (?, ?)";
            $stmt = mysqli_prepare($baglanti, $ekle);
            mysqli_stmt_bind_param($stmt, "ss", $name, $hashed_parola);
